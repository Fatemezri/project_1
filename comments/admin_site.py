...
# ایجاد نمونه از پنل مدیریت سفارشی
moderator_admin_site = ModeratorAdminSite(name='moderator_admin')


# ثبت مدل‌ها با پنل مدیریت سفارشی
...
class CommentModeratorAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        # ناظران فقط نظرات در انتظار و تایید شده را می‌بینند
    ...
    def view_actions(self, obj):
        ...
        if obj.status == 'pending':
            return mark_safe(f"""
                <a href="{moderator_admin_site.reverse('approve_comment', args=[obj.id])}" class="button">تایید</a>
                <a href="{moderator_admin_site.reverse('reject_comment', args=[obj.id])}" class="button">رد</a>
                <a href="{moderator_admin_site.reverse('send_report', args=[obj.id])}" class="button">گزارش</a>
            """)
        return ""
